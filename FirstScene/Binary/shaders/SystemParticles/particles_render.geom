...
void main()
{
	if(iTypePass[0] != 0)
	{
		...
		mat4	mVP		= matrices.mProj*matrices.mView;
			...
			vColorPart	= vec4(vColorPass[0], fLifeTimePass[0]+0.3f);
       
		vec3	vPos	= vPosOld+(-vQuad1-vQuad2)*fSize;
			vTexCoord	= vec2(0.0, 0.0);
			vTexCoord	= vec2(1.0, 1.0);//[mod]
			gl_Position = mVP*vec4(vPos, 1.0);
		EmitVertex();
    
				vPos	= vPosOld+(-vQuad1+vQuad2)*fSize;
			vTexCoord	= vec2(0.0, 1.0);
			vTexCoord	= vec2(1.0, 0.0);//[mod]
			gl_Position = mVP*vec4(vPos, 1.0);
		EmitVertex();
    
				vPos	= vPosOld+(vQuad1-vQuad2)*fSize;
			vTexCoord	= vec2(1.0, 0.0);
			vTexCoord	= vec2(0.0, 1.0);//[mod]
			gl_Position = mVP*vec4(vPos, 1.0);
		EmitVertex();
    
				vPos	= vPosOld+(vQuad1+vQuad2)*fSize;
			vTexCoord	= vec2(1.0, 1.0);
			vTexCoord	= vec2(0.0, 0.0);//[mod]
			gl_Position = mVP*vec4(vPos, 1.0);
		EmitVertex();
